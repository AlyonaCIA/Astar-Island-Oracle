"""
Astar Island — Monte Carlo v2: Vertex AI Single-Script Version
===============================================================

Lean Dilated-CNN Dynamics model from monte_carlo_2.py.

DATA SETUP:
  Upload these folders to a GCS bucket:
    gs://<BUCKET>/simulation_replays/   (50 JSON replay files)
    gs://<BUCKET>/ground_truth/          (50 JSON ground truth files)
    gs://<BUCKET>/observations/          (10 JSON observation files)

USAGE:
  python vertex_ai_mc2.py train-dynamics [--epochs 300] [--reset]
  python vertex_ai_mc2.py train-direct [--epochs 500] [--reset]
  python vertex_ai_mc2.py evaluate [--rollouts 256] [--stride 10]
  python vertex_ai_mc2.py evaluate-direct
"""

# =========================
# 1. INSTALLS (uncomment in notebook)
# =========================
...
REPLAYS_GCS = "simulation_replays"
GT_GCS = "ground_truth"
OBSERVATIONS_GCS = "observations"

# Local paths
BASE_DIR = Path("/home/jupyter/astar_island")
...
CKPT_DIR_DYN     = BASE_DIR / "checkpoints_mc2_dyn"
CKPT_DIR_DIRECT  = BASE_DIR / "checkpoints_mc2_direct"
CKPT_DIR_XGB     = BASE_DIR / "checkpoints_xgb"
...
# =========================
# 4. DOWNLOAD / UPLOAD GCS
# =========================
def download_from_gcs():
    for local_dir, gcs_folder in [
        (LOCAL_REPLAY_DIR, REPLAYS_GCS),
        (LOCAL_GT_DIR, GT_GCS),
        (LOCAL_OBS_DIR, OBSERVATIONS_GCS),
    ]:
        local_dir.mkdir(parents=True, exist_ok=True)
        existing = list(local_dir.glob("*.json"))
        if existing:
            print(f"  {local_dir.name}/: {len(existing)} files already present, skipping")
            continue
        gcs_path = f"gs://{BUCKET}/{gcs_folder}/"
        print(f"  Downloading {gcs_path} -> {local_dir}/")
        ret = os.system(f'gcloud storage cp -r "{gcs_path}*" "{local_dir}/"')
        if ret != 0:
            os.system(f'gsutil -m cp -r "{gcs_path}*" "{local_dir}/"')
        print(f"    Downloaded {len(list(local_dir.glob('*.json')))} files")


def upload_checkpoints_to_gcs():
    for ckpt_dir, gcs_name in [
        (CKPT_DIR_DYN, "checkpoints_mc2_dyn"),
        (CKPT_DIR_DIRECT, "checkpoints_mc2_direct"),
    ]:
        if not ckpt_dir.exists():
            continue
        gcs_path = f"gs://{BUCKET}/{gcs_name}/"
        print(f"  Uploading {ckpt_dir}/ -> {gcs_path}")
        ret = os.system(f'gcloud storage cp -r "{ckpt_dir}/*" "{gcs_path}"')
        if ret != 0:
            os.system(f'gsutil -m cp -r "{ckpt_dir}/*" "{gcs_path}"')
...
def load_all_replays():
    ...
    if not os.path.isdir(d):
        print(f"ERROR: {d} not found"); return replays
...
def _train_dyn_loop(Xt, Tt, Yt, Xv, Tv, Yv, epochs, lr, bs, cw,
    ...
    if save:
        ...
        fp = str(CKPT_DIR_DYN / "dyn_latest.pt")
...
def _train_direct_loop(Xt, Yt, Xv, Yv, epochs, lr, bs,
    ...
    if save:
        meta["total_epochs"] = ep; meta["best_val"] = best_val
        fp = str(CKPT_DIR_DIRECT / "direct_latest.pt")
        torch.save({"epoch": ep, "model_state_dict": model.state_dict(),
                    "optimizer_state_dict": opt.state_dict(),
                    "train_loss": tl, "val_loss": vl, "metadata": meta}, fp)
...
def _extract_pixel_features(grid, h, w):
    """Extract per-pixel features from an initial grid for XGBoost."""
    ...
    # Neighbor counts at various radii (vectorized over the grid)
    neigh_feats = []
    for r in XGB_RADII:
    ...
    for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
        coastal = np.maximum(coastal,
                             padded_ocean[1 + dy:1 + dy + h, 1 + dx:1 + dx + w])
    coastal *= (1.0 - ocean)  # exclude ocean cells themselves

    # Assemble: 8 (onehot) + 3 (pos) + 1 (coastal) + 5*8 (neighbors) = 52
    feats = [
...
def build_xgb_dataset(gt_data, held_out=None):
    """Build per-pixel feature matrix and target probabilities for XGBoost."""
    all_X, all_Y = [], []
    for g in gt_data:
        rid = g.get("_round_id", "")[:8]
        if held_out and rid in held_out:
    ...
    print(f"  XGBoost dataset: {X.shape[0]} pixels, {X.shape[1]} features")
    return X, Y


def train_xgb(gt_data, n_estimators=XGB_N_ESTIMATORS, max_depth=XGB_MAX_DEPTH,
              lr=XGB_LR, reset=False):
    if not HAS_XGB:
        print("ERROR: xgboost not installed. pip install xgboost"); return
    ...
    ckpt_path = CKPT_DIR_XGB / "xgb_class_0.json"
    if not reset and ckpt_path.exists():
        print(f"  Checkpoints already exist in {CKPT_DIR_XGB}/. Use --reset to retrain.")
        return load_xgb_models()

    X, Y = build_xgb_dataset(gt_data)
    if len(X) == 0:
        print("ERROR: no data"); return

    CKPT_DIR_XGB.mkdir(parents=True, exist_ok=True)
    models = []
    t0 = time.time()
    ...
    for c in range(SUBMIT_CLASSES):
        ...
        model.fit(X, Y[:, c])
        model.save_model(str(CKPT_DIR_XGB / f"xgb_class_{c}.json"))
        models.append(model)

    print(f"  All 6 classes trained in {time.time()-t0:.1f}s")
    print(f"  Saved to {CKPT_DIR_XGB}/")
    return models
...
def load_xgb_models():
    ...
    models = []
    for c in range(SUBMIT_CLASSES):
        p = CKPT_DIR_XGB / f"xgb_class_{c}.json"
        if not p.exists():
            return None
        m = xgb.XGBRegressor()
        m.load_model(str(p))
        models.append(m)
    print(f"  Loaded {len(models)} XGBoost models from {CKPT_DIR_XGB}/")
    return models


def predict_xgb(models, grid, h, w):
...
def evaluate_xgb(gt_data, n_estimators=XGB_N_ESTIMATORS,
    ...
    for fi, vr in enumerate(rids):
        ...
        Xt, Yt = build_xgb_dataset(gt_data, held_out={vr})
        if len(Xt) == 0:
            print("    Skip (no training data)"); continue
...
# =========================
# 17. OBSERVATION SCORING & AGGREGATION
# =========================
...
# =========================
# 20. FULL EVALUATION — ROUND K-FOLD
# =========================
...
# =========================
def main():
    ap = argparse.ArgumentParser(description="MC v2: Lean Dilated-CNN (Vertex AI)")
    ...
    if args.cmd == "train-dynamics":
        ...
        g = load_ground_truth()
        if g:
            train_xgb(g, args.n_estimators, args.max_depth, args.lr,
                       args.reset)
    elif args.cmd == "evaluate-xgb":
        g = load_ground_truth()
        if g:
            evaluate_xgb(g, args.n_estimators, args.max_depth, args.lr)
